...
def years_ago(date):
    ...
    Convert = datetime.strptime(date, "%d %b, %Y")
    years = int(Convert.year)
    return THIS_YEAR - years
...
def convert_eu_to_us_date(date):
    date_europ = datetime.strptime(date, "%d/%m/%Y")
    date_american = date_europ.strftime("%m/%d/%Y")
